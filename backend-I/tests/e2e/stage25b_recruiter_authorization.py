...
BASE = os.environ.get("TEST_BASE", "http://127.0.0.1:8077")
...
def mint(user_id):
    import auth
    return auth.create_access_token({"sub": str(user_id)}).decode() \
        if isinstance(auth.create_access_token({"sub": str(user_id)}), bytes) \
        else auth.create_access_token({"sub": str(user_id)})
...
# ---- authz token fixtures (existing rows; tokens minted server-side) ----
ADMIN_TOK = mint(19)
OTHER_TOK = mint(83)
check("admin token mint", bool(ADMIN_TOK), "")
check("other recruiter token mint", bool(OTHER_TOK), "")
...
if OPP and APPX and APPY:
    ...
    before = {r[0]: r for r in db_applications()}
    bx = before[APPX]
    ...
    check("47 client-supplied ids rejected (422)", s == 422,
          "status=%s" % s)
    after = {r[0]: r for r in db_applications()}
    ...
    left = [r for r in db_applications() if r[0] in CREATED["applications"]]
    check("existing application rows intact", all(r[0] in CREATED["applications"] for r in left)
          and len(db_applications()) == len(CREATED["applications"]),
          "apps=%s created=%s" % (len(db_applications()), len(CREATED["applications"])))
...
print("COUNTS_BEFORE:", json.dumps(SM["before"], sort_keys=True))
print("COUNTS_AFTER_SETUP:", json.dumps(SM["after_setup"], sort_keys=True))
print("COUNTS_AFTER_TESTS:", json.dumps(SM["after_tests"], sort_keys=True))
